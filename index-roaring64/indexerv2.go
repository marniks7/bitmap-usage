...
func (s *Holder) IndexPricesV2(catalog *cache64.Catalog) error {

	s.FieldsMetadata = make(map[string]*FieldMetadata)
	s.Index = &Index{Rows: make(map[uint64]*Row)}
...
func (h *Holder) add(fieldName string, fieldValue string) error {
	...
	if !ok {
		...
		metadata.Values = make(map[string]uint64)
...
func (h *Holder) addToBitmap(fieldName string, fieldValue string, ind uint64) error {
	metadata := h.FieldsMetadata[fieldName]
	u := metadata.Values[fieldValue]

	row, ok := h.Index.Rows[u]
	if !ok {
		row = &Row{bitmap: roaring64.New()}
		h.Index.Rows[u] = row
	}
