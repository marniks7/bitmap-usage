...
{"offeringId":"offering3","groupId":"group2","priceSpecId":"spec1","charValues":[{"char":"char2","value":"value2"}],"id":2}{"offeringId":"offering2","groupId":"group2","priceSpecId":"spec1","charValues":[{"char":"char2","value":"value2"}],"id":3}`),
...
func decode(dec *json.Decoder) []model.FindPriceResponseBulk {
	var actual []model.FindPriceResponseBulk
	for dec.More() {
		var resp model.FindPriceResponseBulk
		err := dec.Decode(&resp)
		if err == io.EOF {
			break
		}
		if err != nil {
			//in case if it is not skipped - there will be endless loop
